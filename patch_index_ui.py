...
# 1. Add to Navigation
nav_item = """
                <button onclick="switchTab('scoliosisTab')" id="btn_scoliosisTab" class="w-full sm:w-auto text-left sm:text-center text-slate-500 font-semibold py-3 px-4 border-b-2 border-transparent hover:text-indigo-600 hover:border-indigo-300 transition-colors flex items-center gap-2">
                    <i class="fa-solid fa-bone text-lg"></i> Skolyoz Analizi
                </button>
"""
if "btn_scoliosisTab" not in html:
    html = html.replace('id="btn_footTab"', 'id="btn_footTab"') # Find insertion point
    html = re.sub(
        r'(<button onclick="switchTab\(\'footTab\'\).*?</button>)',
        r'\1' + nav_item,
        html,
        flags=re.DOTALL
    )
        ...
        <div class="flex flex-col lg:flex-row gap-6 mb-8">
            ...
            <div class="w-full lg:w-7/12">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mt-6">
                    <h2 class="text-xl font-bold text-slate-800 mb-4 border-b pb-2">Geçmiş Skolyoz Analizleri</h2>
            ...
            <div class="w-full lg:w-5/12 space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    <div class="bg-slate-50 px-6 py-4 border-b border-slate-200 flex justify-between items-center">
                        <h3 class="font-bold text-slate-800"><i class="fa-solid fa-robot text-indigo-600 mr-2"></i>AI Skolyoz Yorumu</h3>
...
if 'id="scoliosisTab"' not in html:
    html = html.replace('<!-- Modal Overlay -->', scoliosis_tab + '\n    <!-- Modal Overlay -->')
