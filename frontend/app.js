...
async function generateScoliometerQR() {
    ...
    const patientId = currentPatientId; // Assuming this exists globally, else use window.location logic
    if(!patientId) return;

    const btn = document.getElementById('btnGenerateQR');
    btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin mr-2"></i> Oluşturuluyor...';
    btn.disabled = true;

    try {
        const res = await authFetch(`/api/scoliometer/token/${patientId}`, { method: 'POST' });
        if(res.ok) {
            const data = await res.json();
            
            // Generate QR Code
            const qrContainer = document.getElementById('scoliometerQR');
            ...
            btn.innerHTML = '<i class="fa-solid fa-qrcode mr-2"></i> Yeni QR Kod Oluştur';
            btn.disabled = false;
            
            document.getElementById('scoliometerStatus').classList.remove('hidden');
            
            // Start polling for new data
            if(scoliometerPollInterval) clearInterval(scoliometerPollInterval);
            scoliometerPollInterval = setInterval(() => loadScoliometerHistory(patientId), 3000);
            
        }
    } catch (e) {
        console.error(e);
        btn.innerHTML = '<i class="fa-solid fa-qrcode mr-2"></i> QR Kod Oluştur';
        btn.disabled = false;
    }
}

async function loadScoliometerHistory(patientId) {
    if(!patientId) return;
    try {
        const res = await authFetch(`/api/scoliometer/patient/${patientId}`);
        if(res.ok) {
            const data = await res.json();
            const tbody = document.getElementById('scoliometerHistoryList');
            
            if(data.history.length === 0) {
                tbody.innerHTML = '<tr><td colspan="3" class="text-center py-8 text-slate-400">Henüz ölçüm bulunmuyor.</td></tr>';
            } else {
                tbody.innerHTML = '';
                data.history.forEach(row => {
                    let d = new Date(row.date).toLocaleString('tr-TR');
                    let tStr = row.thoracic !== null ? `<span class="bg-orange-100 text-orange-700 px-3 py-1 rounded-lg font-bold">${row.thoracic}&deg;</span>` : '-';
                    let lStr = row.lumbar !== null ? `<span class="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-lg font-bold">${row.lumbar}&deg;</span>` : '-';
                    
                    tbody.innerHTML += `
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="px-4 py-3 text-slate-600 font-medium">${d}</td>
                            <td class="px-4 py-3 text-center">${tStr}</td>
                            <td class="px-4 py-3 text-center">${lStr}</td>
                        </tr>
                    `;
                });
            }
