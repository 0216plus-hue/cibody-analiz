...
async function loadScoliosisHistory() {
    ...
    try {
        ...
        if(res.ok) {
            ...
            if(data.length === 0) {
                list.innerHTML = '<div class="text-center text-slate-400 py-4">Kayıt bulunamadı.</div>';
                return;
            }
            let html = '';
            data.forEach(item => {
                ...
                html += `
                    <div class="p-3 border border-slate-100 rounded-xl hover:bg-slate-50 cursor-pointer flex justify-between items-center" onclick="loadScoliosisAnalysis(${item.id})">
                        <div>
                            <p class="font-bold text-slate-700 text-sm">Cobb Açısı: ${item.cobb_angle || '0'}°</p>
                            <p class="text-xs text-slate-500">${dateStr}</p>
                        </div>
                        <button onclick="event.stopPropagation(); deleteScoliosis(${item.id})" class="text-rose-400 hover:text-rose-600"><i class="fa-solid fa-trash"></i></button>
                    </div>
                `;
            });
            list.innerHTML = html;
